...
module.exports.run = async (client, message, args, queue, searcher) => {
    ...
    if(!vc)
        return message.channel.send("Únete a un chat de voz primero.");
    if (args.length < 1)
        return message.channel.send('Está buena esa "canción".')
    let url = args.join("");
    if(url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)){
        try{
            await ytpl(url).then(async playlist => {
                message.channel.send(`La playlist: "${playlist.title}" ha sido añadida!`)
                playlist.items.forEach(async item => {
                    await videoHandler(await ytdl.getInfo(item.shortUrl), message, vc, true);
                })
            })
        }catch(err){
            return message.channel.send(`Eso no existe: \n${err}`)
        }
    }
    else{
        let result = await searcher.search(args.join(" "), { type: "video" })
        if(result.first == null)
            return message.channel.send("Nope, no hay ningún resultado.");
        try {
        let songInfo = await ytdl.getInfo(result.first.url);
        return videoHandler(songInfo, message, vc)
        }catch(err){
            message.channel.send(`UPS! Algo ha salido mal :c \n ${err} `)
            console.log(err)
        }
    ...
    async function videoHandler(songInfo, message, vc, playlist = false){
        ...
        if(!serverQueue){
            ...
            try{
                ...
                return message.channel.send(`Imposible conectarse: ${err}`)
            }
        }else{
            ...
            let msg = new Discord.MessageEmbed()
                ...
                .addField("Posición", serverQueue.songs.lastIndexOf(song) + 1)
